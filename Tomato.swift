//
//  Tomato.swift
//  Tomato
//
//  Created by Jarrod Norwell on 2/7/2025.
//

public enum GBAKey : UInt8 {
    case a = 0
    case b = 1
    case select = 2
    case start = 3
    case right = 4
    case left = 5
    case up = 6
    case down = 7
    case r = 8
    case l = 9
}

public actor Tomato {
    private var emulator: TomatoEmulator = .shared()
    
    public init() {}
    
    public func insert(_ cartridge: URL) {
        emulator.insert(cartridge)
    }
    
    public func start() {
        emulator.start()
    }
    
    public func pause(_ value: Bool) {
        emulator.pause(value)
    }
    
    public var isPaused: Bool {
        get {
            emulator.isPaused()
        }
        set {
            pause(newValue)
        }
    }
    
    public func stop() {
        emulator.stop()
    }
    
    public func load(state url: URL) {
        emulator.load(url)
    }
    
    public func save(state url: URL) {
        emulator.save(url)
    }
    
    public func framebuffer(_ framebuffer: @escaping (UnsafeMutablePointer<UInt32>) -> Void) {
        emulator.buffer = framebuffer
    }
    
    public func framerate(_ framerate: @escaping (Float) -> Void) {
        emulator.framerate = framerate
    }
    
    public func button(button: GBAKey, player: Int, pressed: Bool) {
        emulator.button(button.rawValue, player: .init(player), pressed: pressed)
    }
}

/*
@MainActor
public class Tomato {
    public var emulator: TomatoObjC = .shared()
    
    public static var shared: Tomato = .init()
    
    public func insert(_ cartridge: URL) {
        emulator.insert(cartridge)
    }
    
    public func start() { emulator.start() }
    public func pause(_ value: Bool) { emulator.pause(value) }
    public func stop() { emulator.stop() }
    
    public func load() { emulator.load() }
    public func save() { emulator.save() }
    
    public func framebuffer(_ framebuffer: @escaping (UnsafeMutablePointer<UInt32>) -> Void) { emulator.buffer = framebuffer }
    public func framerate(_ framerate: @escaping (Float) -> Void) { emulator.framerate = framerate }
    
    public func button(button: GBAKey, player: Int, pressed: Bool) { emulator.button(button.rawValue, player: .init(player), pressed: pressed) }
}
*/
