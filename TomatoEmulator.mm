...
void SWVideoDevice::Draw(u32* buffer) {
    if (auto framebuffer = [[TomatoEmulator sharedInstance] buffer]) {
...
+(TomatoEmulator *) sharedInstance {
    static TomatoEmulator *sharedInstance = NULL;
...
-(void) insertCartridge:(NSURL *)url {
    ...
    object.thread->SetFrameRateCallback([](float fps) {
        if (auto framerate = [[TomatoEmulator sharedInstance] framerate]) {
            dispatch_async(dispatch_get_main_queue(), ^{
                framerate(fps);
            });
        }
    ...
    nba::BIOSLoader::Load(object.core, [[[directory URLByAppendingPathComponent:@"sysdata"] URLByAppendingPathComponent:@"bios.bin"].path UTF8String]);
    nba::ROMLoader::Load(object.core, [url.path UTF8String], [[[directory URLByAppendingPathComponent:@"saves"] URLByAppendingPathComponent:[name stringByAppendingString:@".sav"]].path UTF8String]);
}

-(void) start {
    object.core->Reset();
    object.thread->Start(std::move(object.core));
}

-(void) pause:(BOOL)paused {
    object.thread->SetPause(paused);
}

-(BOOL) isPaused {
    return object.thread->IsPaused();
}
...
-(void) load:(NSURL *)url {
    auto wasRunning = object.thread->IsRunning();
    object.core = object.thread->Stop();
    
    nba::SaveStateLoader::Load(object.core, [url.path UTF8String]);
...
-(void) save:(NSURL *)url {
    ...
    nba::SaveStateWriter::Write(object.core, [url.path UTF8String]);
...
-(void) button:(uint8_t)button player:(int)player pressed:(BOOL)pressed {
    object.thread->SetKeyStatus((nba::Key)button, pressed);
}
@end
